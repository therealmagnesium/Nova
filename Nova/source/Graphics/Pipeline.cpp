...
{
    static constexpr u8 pipeline_count = static_cast<u8>(PipelineType::_Length);
    ...
    void Shutdown()
    {
        ...
        for (u8 i = 0; i < static_cast<u8>(PipelineType::_Length); i++)
            if (pipelines[i] != NULL)
    ...
    void InitOutdoorMeshes(const Shader* shader)
    {
        ...
        // --- Create the pipeline ---
        const u8 index = static_cast<u8>(PipelineType::OutdoorMeshes);
    ...
    void InitOutdoorMeshesSkinned(const Shader* shader)
    {
        ...
        // --- Create the pipeline ---
        const u8 index = static_cast<u8>(PipelineType::OutdoorMeshesSkinned);
        pipelines[index] = SDL_CreateGPUGraphicsPipeline(device, &pipeline_info);
        if (pipelines[index] == NULL)
        {
    ...
    void InitIndoorMeshes(const Shader* shader)
    {
        ...
        // --- Create the pipeline ---
        const u8 index = static_cast<u8>(PipelineType::IndoorMeshes);
        pipelines[index] = SDL_CreateGPUGraphicsPipeline(device, &pipeline_info);
        if (pipelines[index] == NULL)
        {
    ...
    void InitWireframeMeshes(const Shader* shader)
    {
        ...
        // --- Create the pipeline ---
        const u8 index = static_cast<u8>(PipelineType::WireframeMeshes);
        pipelines[index] = SDL_CreateGPUGraphicsPipeline(device, &pipeline_info);
        if (pipelines[index] == NULL)
        {
